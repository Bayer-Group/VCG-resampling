...
resampling_result_baycal_plot <- function(
  ...
  #*If the data frame is empty, filter functions won't work. Therefore, an
  #*if-statement needs to be done so that a NULL-value will be returned if
  #*I try to filter from NULL
  # checknullfirst <- function(df_to_filter, action){
  #   if(is.null(df_to_filter) | identical(df_to_filter, character(0))){NULL}
  #   else{df_to_filter %>% action}
  # }
  ...
  plot_ccg_vs_VCGs <- plot_ly() %>%
    ...
    layout(
      plot_bgcolor="#e5ecf6",
      font = list(size = 18),
      #add a horizontal line to the right of the plot as a separator
      # shapes = list(
      #   type = "line",
      #   y0 = -.1,
      #   y1 = 1,
  ...
  # #*Make a box plot showing the distributions of the data sets which you
  # #*can take samples from
  # #plot K with respect to narcosis
  # plot_sample_population <- plot_ly() %>%
  #   add_boxplot(
  #     x = "Sample population",
  #     y = sampling_group,
  #     opacity = 0.8,
  #     marker = list(color = "511cfb", size = 7, opacity = 0.3),
  #     line = list(color = "511cfb"),
  #     fillcolor = list(color = "511cfb"),
  #     boxpoints = "all", jitter = "0.7", pointpos = "0",
  #     name = "sample population",
  #     legendgroup = "sample_population"
  #   ) %>%
  # #Add the count of each group
  # add_annotations(
  #   text = ~paste0("<i>n</i><sub>sample</sub>: ", length(sampling_group)),
  #   size = 18,
  #   x = 0.5,
  #   y = 1,
  #   xref = "paper",
  ...
  #       line = list(color = "black")
  #     ),
  #     # title = paste0(titleelectrolyte, " levels of ", titlesex, " Wistar-rats with respect to anesthetics"),
  #     xaxis = list(title = "",
  #                  zerolinecolor = "#ffff", 
  #                  zerolinewidth = 2, 
  #                  gridcolor = "ffff",
